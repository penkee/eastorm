...
public class CityController extends BaseController {
	...
	public ModelAndView getMenuList(@PathVariable("parentId") Integer parentId,
			...
			HttpSession session) {
		ModelAndView mav = new ModelAndView("back/sys/sysCityList");
		 ServletContext servletContext = super.getServletContext();
		 //-1标记为根
		if(-1==parentId){
			parentId=(Integer)servletContext.getAttribute("cityRootId");
		}
		mav.addObject("parentId", parentId);

		Integer pId=null;
		if(-1!=parentId&&parentId.equals((Long)servletContext.getAttribute("categoryId"))){
			//查找父节点,根没有父节点
	...
	public Map<String,Object> update(@PathVariable("parentId") Integer parentId,
			HttpEntity<SysCity> sysCity){
		//页面传来的数据
		SysCity formitem=sysCity.getBody();	
		boolean res = true;
		try {			
			if (formitem.getId() == null) {
				...
				//修改
				SysCity item= sysCityService.findOne(formitem.getId());
				...
				sysCityService.save(item);
			}
		} catch (Exception e) {
			res=false;
			e.printStackTrace();
			logger.debug("插入城市失败："+e);
		}
		Map<String,Object> map = new HashMap<String,Object>();  
        map.put("success",res);  
        map.put("message","" );  
	...
	@ResponseBody
	public Map<String,Object> delete(SysCity sysCity){
		//删除之前判断是否有孩子
		Map<String,Object> map = new HashMap<String,Object>();  
		if(nodeLinkService.qryListCountByParentId(sysCity.getId())>0){
			 map.put("status",100);  
		}else{
			try {
				sysCityService.delete(sysCity.getId());
	...
	public Map<String,Object> refresh(){
		...
		try {
			...
			sysCityService.queryAll(Integer.parseInt(cityRootId));
