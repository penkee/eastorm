   ...
   <!-- 包含extjs基本库文件-->
	...
	Ext.onReady(function(){
	    ...
	    var store = Ext.create('Ext.data.Store', {
	        ...
	        pageSize: 20,  //一页显示多少行
	        model: 'DictItemModel',//对应上边的model
	        remoteSort: true,
	        proxy: {
	            type: 'ajax',//rest指的是method有get,post,put,delete;ajax指的是get,post
	            api:{
	                read:'dictItem/read',
	                update:'dictItem/update',//store中的某条记录发生更改，会将更改反馈给该url
	                create:'dictItem/update'
	            },
	            reader: {
	            	type:'json',
	                root: 'list',
	    ...
	    var grid = Ext.create('Ext.grid.Panel', {
	        ...
	        dockedItems: [{//菜单插件，可定位的项
	            ...
	            items: [{
	                ...
	                handler: function(){
	                    // empty record
	                    store.insert(0, new DictItemModel());
	                ...
	                handler: function(){
	                	 Ext.Ajax.request({ 
                				url: "dictItem/refresh", 
                				method: "POST", 
                				callback: function(options,success,response) { 
                					var jsonResult = Ext.JSON.decode(response.responseText);    
                					switch(jsonResult.status){
                						default:Ext.Msg.alert(jsonResult.message,'成功');break;
                					}     
                				}, 
                				failure: function(response) { 
                    				Ext.Msg.alert("警告", "数据操作失败，请稍后再试！"); 
